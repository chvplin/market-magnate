...
(function () {
  ...
  window.mmItemIconImgErr = function (img) {
    if (!img) return;
    try {
      var folder = img.getAttribute("data-mm-icon-folder") || "";
      if (folder && !img.dataset.mmTriedCatFb) {
        img.dataset.mmTriedCatFb = "1";
        var catFb = window.mmIconFallbackPath && window.mmIconFallbackPath(folder);
        if (catFb && img.src !== catFb) {
          img.src = catFb;
          return;
        }
      }
    ...
    try {
      ...
      if (misc && img.src !== misc && !img.dataset.mmTriedMiscFb) {
        img.dataset.mmTriedMiscFb = "1";
        img.src = misc;
        return;
      }
    } catch (e1) {}
