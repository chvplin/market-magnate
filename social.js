(function () {
  ...
  window.MMGameAdapter = window.MMGameAdapter || {
    ...
    getStats(saveData) {
      ...
      const netWorth = Number(g.netWorth || (cash + portfolio));
      return {
        net_worth: netWorth,
        cash,
        total_profit: Number(g.totalEarned || g.totalProfit || 0),
        lifetime_trades: Number(g.trades || g.totalTrades || 0),
        highest_net_worth: Number(g.highestNetWorth || netWorth),
        prestige: Number(g.legacy || g.prestige || 0),
        xp: Number(g.xp || 0),
