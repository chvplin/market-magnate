(function () {
  ...
  async function signUp() {
    ...
    if (!user) {
      flash(el.authMessage(), "Signup started. Check your email if confirmation is required.");
      return;
    }

    await window.mmSupabase.from("profiles").upsert({
      id: user.id,
      username,
    ...
    flash(el.authMessage(), "Account created.");
    refreshAuthUI();
    loadMyProfile();
    loadLeaderboard();
  }
